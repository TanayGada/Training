...
//cases
// 1. a[i] is between b[i]-k and b[i]+k -> valid 
// 2. a[i] is before b[i]-k -> need a larger a[i]
// 3. a[i] is after b[i]+k -> need a larger b[i]
...
void solve()
{
    ...
    while(left1<n && left2<m){
        if(a[left1]<=(b[left2]+k) && a[left1]>=(b[left2]-k)){
            cnt++;
            left1++;
            left2++;
        }
        else if(a[left1]>(b[left2]+k)){
            left2++;
        }
        else if(a[left1]<b[left2]-k){
            left1++;
