...
// if we have ticketPrice equal to max a person can give, we sell it
// else give the next lower than the max a person can give
// we think from the perspective of the ticket seller 
...
void solve()
{
    ...
    for(int i=0; i<m; i++){ // O(N)
        auto ub = a.upper_bound(b[i]); // O(logN)
        if(ub==a.begin()){
            ans[i] = -1;
        }
        else{
