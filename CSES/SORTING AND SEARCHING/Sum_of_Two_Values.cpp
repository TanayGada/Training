...
// if curr ele is a[i], we need to find x-a[i] element
// case to handle: a[i] == x-a[i], same indexes cannot be ans
...
void solve()
{
    ...
    for(auto i : mp){
        auto it = mp.find(x-i.first);
        if(it==mp.end());
        else{
            vector<int> ind = it->second;
            if(ind.size()==1 && i.first == it->first);
            else{
                int first = i.second[0];
                int second = it->second[0];
                if(ind.size()>1)    second = ind[1];
