// memoized
// from index currInd=n, for every word in wordDict, if word matches substring(currInd-word.length()-1, currInd),
// mark index currInd as true. and repeat same for currInd = currInde-word.length()
// recurrence relation: dp[i]=any(s[i - word.length + 1, i]==word && dp[i - word.length])
// tc -> wordDict * wordLength * length(s)
// sc -> length(s)
class Solution {
public:
    vector<int> dp;
    ...
    bool func(int ind){
        ...
        for(string word : wordDict){
            ...
            if(ind-currSize+1<0)    continue;
            if(s.substr(ind-currSize+1, currSize)==word && func(ind-currSize)){
                dp[ind] = 1;
                return true;
            }
        }
