...
class Solution {
public:
// func(ind, stock) -> max profit till index ind with curr stock holding
    int func(int ind, int stock, vector<int>& prices, vector<vector<int>>& dp){
        if(ind==prices.size())  return 0;
        if(dp[ind][stock]!=-1)  return dp[ind][stock];
        if(stock){ //own a stock
            int sell = prices[ind]+func(ind+1, 0, prices, dp);
            int hold = func(ind+1, 1, prices, dp);
            return dp[ind][stock] = max(sell, hold);
        }else{
            int buy = max(0, -prices[ind]+func(ind+1, 1, prices, dp));
            int nothing = func(ind+1, 0, prices, dp);
        ...
        for(int ind=n-1; ind>=0; ind--){
            for(int stock=0; stock<=1; stock++){
                if(stock){ //own a stock
                    ...
                    int buy = max(0, -prices[ind]+dp[ind+1][1]);
                    int nothing = dp[ind+1][0];
...
public:
    int maxProfit(vector<int>& prices) {
        ...
        for(int ind=n-1; ind>=0; ind--){
            ...
            for(int stock=0; stock<=1; stock++){
                if(stock){ //own a stock
                    int sell = prices[ind]+next[0];
                    int hold = next[1];
                    curr[stock] = max(sell, hold);
                }else{
                    int buy = max(0, -prices[ind]+next[1]);
                    int nothing = next[0];
