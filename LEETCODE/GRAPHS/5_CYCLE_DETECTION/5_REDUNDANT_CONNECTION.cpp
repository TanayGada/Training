// identify the first node to take part in a cycle using dfs -> O(V+E)
// backtrack to get all nodes in the cycle -> O(V+E)
// return the edge between nodes which have the cycle -> O(E)

// tc -> O(V+E), sc -> O(V+E)
// more intuitive using DSU
